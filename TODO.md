...
- 简单 alias 会回溯到同一 released 资源，并覆盖 `*p` 解引用和释放后 return；由
  `failed-acquire` 精化产生的 RELEASED 不是实际 free/close，不作为 UAF。
...
### D11 acquire wrapper + owned overwrite 补全（通用粗筛）
- `discover_acquire_wrappers()` 自动推断返回已知 owned resource 的项目函数，支持直接
  `return malloc()`、返回 active 局部变量、跨文件和多层 fixpoint；static 按文件隔离。
- `owned_overwrite` 覆盖 return-value acquire、out-param acquire，以及 active handle 被
  普通新值覆盖；`realloc(p, ...)` 自更新不按普通 lost-handle 报告。
- UAF 补充简单 alias、`*p` 和 dangling return，并区分真实 release 与 failed-acquire。

当前 65 个测试全部通过。6 个真实仓库默认扫描均零 warning；新增 UAF 在 lwIP 留下 2 个
候选（复杂 switch/条件编译与局部重绑定导致，交给 LLM 复核），Paho UAF 为 0。
...
### P3 use_after_release（释放后使用）—— 已实现（见 D9/D11）
没走 JSON-use-API-列表那条会要数据的路，而是直接复用资源数据流的 `RELEASED` 状态做通用
后置检查（传给调用 / `*p`/`->`/`[]` 解引用 / dangling return）。全自动、零规格。
...
queue/task/timer…）、`double_release`、`use_after_release`、`owned_overwrite`、
...
65 个测试全绿，六个真实仓库默认扫描零 warning，`analysis/` 与 jni 字节一致。
